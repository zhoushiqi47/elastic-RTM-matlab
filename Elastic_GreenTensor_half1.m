function G=Elastic_GreenTensor_half1(omega,kp,ks,x,y)
%x is point source
% 2016 6 22 
...
B2=@(t) -2*t.^2.*Sp(t).*exp(Ss(t)*x(2,:)+1i*t*(y(1,:)-x(1,:)))-beta(t).*Sp(t).*exp(Sp(t)*x(2,:)+1i*t*(y(1,:)-x(1,:)));
    ...
    G(1,:)=quadv(@(t) A1(t).*B1(t) ,-l,l)+G1(1,:)+G2(1,:);
    G(2,:)=quadv(@(t) A2(t).*B1(t) ,-l,l)+G1(2,:)+G2(2,:);
    G(3,:)=quadv(@(t) A1(t).*B2(t) ,-l,l)+G1(3,:)-G2(3,:);
    G(4,:)=quadv(@(t) A2(t).*B2(t) ,-l,l)+G1(4,:)-G2(4,:);
