function G=Elastic_GreenTensor_Thalf(omega,kp,ks,x,y)
...
for j=1:n(2)
    ...
    G(1,j)=integral(@(t) firstG(t,x(:,j),y(:,j),w2,k1,k2) ,-l,l)+G1(1,j)+G2(1,j);
    G(2,j)=integral(@(t) secondG(t,x(:,j),y(:,j),w2,k1,k2) ,-l,l)+G1(2,j)+G2(2,j);
    G(3,j)=integral(@(t) thirdG(t,x(:,j),y(:,j),w2,k1,k2) ,-l,l)+G1(3,j)-G2(3,j);
    G(4,j)=integral(@(t) forthG(t,x(:,j),y(:,j),w2,k1,k2) ,-l,l)+G1(4,j)-G2(4,j);
